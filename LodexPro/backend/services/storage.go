...
func (s *Storage) GetAllTasks() ([]*models.DownloadTask, error) {
	rows, err := s.db.Query(`SELECT id FROM downloads ORDER BY date_created DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Storage) SaveQueue(q *models.DownloadQueue) error {
	...
	// Ensure post_action column exists
	...
	`, q.ID, q.Name, q.IsDefault, q.MaxConcurrent, q.StartTime, q.StopTime, q.IsScheduled, string(daysBytes), string(tasksBytes), q.PostAction)
	return err
}

func (s *Storage) GetQueues() ([]*models.DownloadQueue, error) {
	rows, err := s.db.Query(`
		SELECT id, name, is_default, max_concurrent, start_time, stop_time, is_scheduled, days_of_week, task_ids, IFNULL(post_action, '') 
	...
	for rows.Next() {
		...
		err := rows.Scan(&q.ID, &q.Name, &q.IsDefault, &q.MaxConcurrent, &q.StartTime, &q.StopTime, &q.IsScheduled, &daysStr, &tasksStr, &q.PostAction)
		if err == nil {
			json.Unmarshal([]byte(daysStr), &q.DaysOfWeek)
			json.Unmarshal([]byte(tasksStr), &q.TaskIDs)
			if q.DaysOfWeek == nil { q.DaysOfWeek = []int{} }
			if q.TaskIDs == nil { q.TaskIDs = []string{} }
